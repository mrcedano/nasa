...
async function seleccionarOpción(index) {
  ...
  if (validezRespuesta) {
    await Swal.fire({
      ...
      text: "¡Buen trabajo viajero espacial!",
      icon: "success",
    });
    puntaje++;
  } else {
    await Swal.fire({
      ...
      html: `Lo siento pero la respuesta correcta es: "${objetoPregunta.respuesta}"`,
      icon: "error",
    });
  }
  INDEX_PREGUNTA++;
  if (INDEX_PREGUNTA >= baseDePreguntas.length) {
    await Swal.fire({
      title: "Viaje Términado",
      text: `Tu puntaje espacial fue de: ${puntaje}/${baseDePreguntas.length}`,
    });
    INDEX_PREGUNTA = 0;
    puntaje = 0;
  }
